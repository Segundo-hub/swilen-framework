...
dataset('valid-tokens', [
    (new Jwt())->sign(['exp' => time() + 20, 'data' => ['userId' => uniqid()]], STATIC_SECRET, 'HS256')->token,
    (new Jwt())->sign(['exp' => time() + 40], STATIC_SECRET, 'HS256')->token,
    (new Jwt())->sign(['exp' => time() + 30], STATIC_SECRET, 'HS256')->token,
]);
...
it('Create Jwt and verify token generated', function () {
    ...
    $result = (new Jwt())->verify($jwt->token, STATIC_SECRET, 'HS256');
...
it('Jwt Error time expired', function () {
    ...
    (new Jwt())->verify($jwt->token, STATIC_SECRET, 'HS256');
...
it('Throw error when token is altered', function () {
    ...
    $jwt = (new Jwt())->verify($random->token, 'secret', 'HS512');
...
it('Decoder token match with token generated by Jwt', function () {
    ...
    expect((new Decoder($generated->token))->token())->toBe($generated->token);
});

function alterJwt(JwtSignedExpression $expresion)
{
    $decode = (new Decoder($expresion->token))->decode();
